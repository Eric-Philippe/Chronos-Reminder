...
fun RegisterScreen(
    uiState: AuthUiState,
    onRegister: (email: String, username: String, password: String, timezone: String) -> Unit,
    ...
    Scaffold(
        ...
        topBar = { ChronosTopBar(title = stringResource(R.string.register_title), onBack = onBack) },
    ) { padding ->
        Box(
            modifier = Modifier
                .fillMaxSize()
            ...
            Column(
                ...
                ChronosButton(
                    ...
                    onClick = {
                        localError = when {
                            ...
                            password.isBlank() -> passwordRequiredError
                            password != passwordConfirm -> mismatchError
                            else -> null
                        }
                        if (localError == null) {
                            onRegister(email, username, password, timezone)
                        }
                    },
                    modifier = Modifier.fillMaxWidth(),
                ...
                TextButton(onClick = onBack, modifier = Modifier.align(Alignment.CenterHorizontally)) {
                    Text(
                        stringResource(R.string.have_account_sign_in),
...
@Composable
fun TimezonePickerSheetContent(
    timezones: List<com.chronos.reminder.account.data.TimezoneDto>,
    onSelect: (String) -> Unit,
) {
    var query by rememberSaveable { mutableStateOf("") }
    val filtered = timezones
        .filter { it.ianaLocation.contains(query, ignoreCase = true) }
    ...
    Column(modifier = Modifier.padding(horizontal = 16.dp)) {
        ChronosTextField(
            ...
            modifier = Modifier.fillMaxWidth(),
            placeholder = stringResource(R.string.search_timezones),
        )
        Spacer(Modifier.height(8.dp))
        LazyColumn {
            items(filtered, key = { it.ianaLocation }) { tz ->
                val sign = if (tz.gmtOffset >= 0) "+" else ""
                Text(
                    text = "${tz.ianaLocation} (GMT$sign${tz.gmtOffset})",
                    style = MaterialTheme.typography.bodyLarge,
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable { onSelect(tz.ianaLocation) }
